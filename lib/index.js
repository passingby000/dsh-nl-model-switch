 ...
 * (e.g. "切到 deepseek-v4-flash 模型做翻译任务"), independent of any IM bridge.
 *
 * Design:
 ...
 *    model sends the "已从 A 切换到 B" confirmation and continues the user's task.
 *
...
export function apply(ctx) {
  ...
  const switchModelTool = defineTool({
    ...
    description:
      ...
      '(e.g. "切到 deepseek-v4-flash 模型", "切到 xxx", "换到 xxx 做任务", "switch to xxx model"). ' +
    ...
    async execute(args, exec) {
      ...
      if (!modelsResponse?.result?.ok) {
        ...
        throw new Error(`session.models failed: ${error ?? 'unknown error'}`);
      }
      ...
      if (!selectResponse?.result?.ok) {
        ...
        throw new Error(`session.selectModel failed: ${error ?? 'unknown error'}`);
      }
  ...
  ctx.get('systemPrompt').section({
    ...
    text: [
      ...
      '(e.g. "切到 xxx 模型", "切到 xxx", "换到 xxx 做任务", "用 xxx 模型", "switch to xxx model", "use model xxx"),',
      'call `switch_model` with the requested (registered) model id, then continue with any task the user asked for.',
      'After the switch resolves, explicitly confirm the switch (e.g. "已从 xxx 切换到 xxx") and continue the original request.',
    ].join(' '),
  });
